...
class mySSA(object):
    ...
    def diagonal_averaging(self ,hankel_matrix):
    ...
    def view_time_series(self):
        ...
        self.ts.plot(title='Original Time Series',label=False)
        plt.show()
    ...
    def view_s_contributions(self, adjust_scale=False, cumulative=False, return_df=False):
        ...
        ax.set_title('Non-zero{} contribution of Singular{}'.\
...
if __name__=='__main__':
    ...
    for run_time in range(int(data_num)):
        ts = pd.read_csv("./EMD_sequence/series_sub_real_time"+str((run_time)+1)+"_new1.csv", parse_dates=True, index_col='Unnamed: 0')
        #ts = pd.read_csv("./EMD_sequence/series_sub_real_time"+str((run_time)+1)+".csv", parse_dates=True, index_col='Unnamed: 0')
        ts_ = ts.copy()
        ...
        ssa.embed(embedding_dimension=100, suspected_frequency=57, verbose=True)
        ssa.decompose(True)
        ssa.view_s_contributions(adjust_scale=False)
    '''
    # Component Signals
    components = [i for i in range(99)]
