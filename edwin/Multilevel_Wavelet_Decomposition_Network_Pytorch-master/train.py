...
import numpy as np
from utils import plot_results,ToVariable,use_cuda,TorchDataLoader, plot_predict

def train(model,x_train,y_train,epochs=1,batch_size=256,alpha=0.3,beta=0.3):
...
def test(model,x_test,y_test,data_df_combined_clean):
    ...
    plot_predict(preds_unstd[-5:,-1,-5:])
